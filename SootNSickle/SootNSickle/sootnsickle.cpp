...
void SootNSickle::initialize(HWND hwnd)
{
	...
	for(int i = 0; i < SootNSickleNS::NUM_MENU_OPTIONS; i++)
	{
	...
	if(!player.alive)
	{
		...
		if(playerDeathCountdown<=0)
			if(numLives > 0)
			{
				switch (currentState)
				{
				case SootNSickle::Level1:
					level1Load();
					break;
				case SootNSickle::Level2:
					level2Load();
					break;
				case SootNSickle::Level3:
					level3Load();
					break;
				case SootNSickle::FeelingLucky:
					feelingLuckyLoad();
					break;
				default:
					break;
	...
	if(currentState!=TitleScreen)
	{
		...
		if(player.collidesWith(exit,collisionVector))
		{
			...
			case SootNSickle::Level3:
				...
				menuLoad();
				break;
			case SootNSickle::RestartScreen:
				break;
			case FeelingLucky:
				menuLoad();
			default:
...
void SootNSickle::updateScreen(VECTOR2 center)
{
...
Bullet* SootNSickle::spawnBullet(VECTOR2 loc, float dir,COLOR_ARGB c, bool playerBullet)
{
...
Particle* SootNSickle::spawnParticle(VECTOR2 loc,VECTOR2 vel, COLOR_ARGB c)
{
	for(int i = 0; i < MAX_PARTICLES; i++)
	{
...
Turret* SootNSickle::spawnTurret(VECTOR2 loc, float dir)
{
	for(int i = 0; i < MAX_TURRETS; i++)
	{
...
Guard* SootNSickle::spawnGuard(VECTOR2 loc)
{
	for(int i = 0; i < MAX_GUARDS; i++)
	{
...
Wall* SootNSickle::spawnWall(VECTOR2 loc, VECTOR2 size)
{
	for(int i = 0; i < MAX_WALLS; i++)
	{
...
LandMine* SootNSickle::spawnMine(VECTOR2 loc)
{
	for(int i = 0; i < MAX_MINES; i++)
	{
...
Item* SootNSickle::spawnItem(VECTOR2 loc, Item::ItemType t)
{
	for(int i = 0; i < MAX_ITEMS; i++)
	{
...
Item* SootNSickle::getItemUnderPlayer()
{
	VECTOR2 v;
	for(int i = 0; i < MAX_ITEMS; i++)
	{
...
VECTOR2 SootNSickle::getRealEndLoc(VECTOR2 startLoc, VECTOR2 endLoc)
{
	for(int i = 0; i < MAX_WALLS; i++)
	{
		if(walls[i].getActive())
...
void SootNSickle::onPlayerDeath()
{
